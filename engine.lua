...
local function Topology_Cylinder(id)
    ...
    return x, y, z, yaw, 0 -- x, y, z, yaw, pitch
end

local ActiveTopology = Topology_Cylinder

-- ==========================================
-- GEOMETRY BUILDER (Replaces slides_internal)
-- ==========================================
local function BuildSlideMesh(x, y, z, w, h, thickness, color)
    -- Create the base FFI allocation
...
-- ==========================================
-- SCENE ASSEMBLY LINE (Replaces slide_guard)
-- ==========================================
local function ProcessSlideNode(id, data)
    -- 1. Setup Basic Data & Defaults
    ...
    -- 4. Write to Logical Box Buffers (For collisions & text rendering)
    ...
    -- 7. Sync the Geometry Transform to match the Box logic
...
function Engine.Boot(json_path)
    ...
    -- Explicit, readable error checks
    if not content then
    ...
    local count = 0
    -- Build a local table for the text strings
    local textPayload = {}
    ...
    -- Return textPayload instead of manifest
    return {
