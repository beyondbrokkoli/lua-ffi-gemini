...
local MAX_OBJS = 2048
local Crystal_HomeIdx = ffi.new("int[?]", MAX_OBJS) -- Collision Checks
...
SlidesInternal.CrystalCompanion = function(api, manifest, numSlides, crystalsPerSlide)
    ...
    for i = 0, numSlides - 1 do
        ...
        for j = 1, crystalsPerSlide do
            ...
            -- Increased padding to prevent spawning inside repulsion box
...
local function NO_COLLISION_BatchUpdateTransforms(dt)
...
local function BatchUpdateTransforms(dt)
    for i = 0, NumObjects - 1 do
        ...
        if Is_Crystal[i] then
            ...
            -- CULPRIT: 2250000 was static.
            -- FIX: Threshold now scales with the slide size.
            ...
            if math.abs(localX) < boxW and math.abs(localY) < boxH and math.abs(localZ) < boxD then
                local pushZ = (localZ > 0 and 200 or -200) * dt
                Obj_VelX[i] = Obj_VelX[i] + (nx * pushZ)
                Obj_VelZ[i] = Obj_VelZ[i] + (nz * pushZ)

                -- Dampen ALL axes to stop the "up/down stream" effect
                Obj_VelX[i] = Obj_VelX[i] * 0.95
                Obj_VelY[i] = Obj_VelY[i] * 0.95
                Obj_VelZ[i] = Obj_VelZ[i] * 0.95
            end
        end
...
function love.load()
    ...
    local slideAPI = {
        ...
        Crystal_HomeIdx = Crystal_HomeIdx,
