...
function CreateTorusKnot(cx, cy, cz, scale, tubeRadius, p, q, segments, sides)
    local obj = CreateTriObject(cx, cy, cz, segments * sides, segments * sides * 2)
    local vIdx, tIdx = 0, 0

    for i=0, segments-1 do
        local t = (i / segments) * math.pi * 2

        -- Torus Knot Parametric Equations (P, Q)
        local r = cos(q * t) + 2
        local tx, ty, tz = r * cos(p * t), r * sin(p * t), -sin(q * t)

        -- Tangent calculation for tube orientation
        local dt = 0.001
        local r2 = cos(q * (t+dt)) + 2
        local fx = (r2 * cos(p * (t+dt))) - tx
        local fy = (r2 * sin(p * (t+dt))) - ty
        local fz = (-sin(q * (t+dt))) - tz
        local len = sqrt(fx*fx + fy*fy + fz*fz)
        fx, fy, fz = fx/len, fy/len, fz/len

        -- Coordinate frame for tube cross-section
        local nx, ny, nz = 1, 0, 0
        if math.abs(fx) > 0.9 then nx, ny, nz = 0, 1, 0 end

        local rx = fy*nz - fz*ny; local ry = fz*nx - fx*nz; local rz = fx*ny - fy*nx
        local rlen = sqrt(rx*rx + ry*ry + rz*rz); rx,ry,rz = rx/rlen, ry/rlen, rz/rlen
        local ux = fy*rz - fz*ry; local uy = fz*rx - fx*rz; local uz = fx*ry - fy*rx

        for j=0, sides-1 do
            local ph = (j/sides) * math.pi * 2
            local sx, sy = cos(ph) * tubeRadius, sin(ph) * tubeRadius

            obj.vx[vIdx] = (tx * scale) + (sx * rx + sy * ux)
            obj.vy[vIdx] = (ty * scale) + (sx * ry + sy * uy)
            obj.vz[vIdx] = (tz * scale) + (sx * rz + sy * uz)
            vIdx = vIdx + 1
        end
    end
    ...
    for i=0, segments-1 do
        ...
        for j=0, sides-1 do
            ...
            obj.tris[tIdx] = {v1=a, v2=c, v3=b, color=0xFFCCEEFF}; tIdx = tIdx + 1
            obj.tris[tIdx] = {v1=a, v2=d, v3=c, color=0xFF77AACC}; tIdx = tIdx + 1
        end
    end
    return obj
end

function CreateTrefoil(cx, cy, cz, scale, tubeRadius, segments, sides)
    local obj = CreateTriObject(cx, cy, cz, segments * sides, segments * sides * 2)
    local vIdx, tIdx = 0, 0

    for i=0, segments-1 do
        local t = (i / segments) * math.pi * 2
        local tx = sin(t) + 2 * sin(2 * t)
        local ty = cos(t) - 2 * cos(2 * t)
        local tz = -sin(3 * t)

        local dt = 0.01
        local tx2 = sin(t+dt) + 2 * sin(2*(t+dt))
        local ty2 = cos(t+dt) - 2 * cos(2*(t+dt))
        local tz2 = -sin(3*(t+dt))

        local fx, fy, fz = tx2-tx, ty2-ty, tz2-tz
        local len = sqrt(fx*fx + fy*fy + fz*fz)
        fx, fy, fz = fx/len, fy/len, fz/len

        local rx, ry, rz = fy, -fx, 0
        local rlen = sqrt(rx*rx + ry*ry + rz*rz)
    ...
    for i=0, segments-1 do
        ...
        for j=0, sides-1 do
            local j_next = (j+1) % sides
            local a, b, c, d = i*sides+j, i_next*sides+j, i_next*sides+j_next, i*sides+j_next
            obj.tris[tIdx] = {v1=a, v2=c, v3=b, color=0xFF55AAFF}; tIdx = tIdx + 1
            obj.tris[tIdx] = {v1=a, v2=d, v3=c, color=0xFF3366CC}; tIdx = tIdx + 1
        end
    end
    return obj
end
...
function love.load()
    ...
    -- Position Camera back so we can see the "Exhibit"
    Cam.pos = {x=0, y=0, z=-400}
--    Cam.pos = {x=150, y=50, z=-100}
    Cam.yaw = 0
    Cam.pitch = 0

    -- 1. The Classic Torus (Gold)
    CreateTorus(-150, 0, 100, 50, 20, 64, 32)

    -- 2. The Trefoil Knot (Electric Blue)
    CreateTrefoil(0, 80, 200, 30, 8, 128, 24)

    -- 3. The Torus Knot (Cinematic Silver/White)
    -- p=3, q=7 creates a very intricate "spiky" knot
    CreateTorusKnot(150, -50, 150, 25, 5, 3, 7, 256, 16)

    -- 4. A Tiny "Moon" Torus orbiting the center
    local moon = CreateTorus(0, 0, 0, 15, 5, 32, 16)
    moon.transform.pos = {x=0, y=-120, z=50}
end
