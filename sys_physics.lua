...
function Physics.ResolveCollisions()
    for i = 0, Pool_SlideCollider_Count - 1 do
        ...
        if px < B_MinX then px = B_MinX
            vx = abs(vx) end
        if px > B_MaxX then px = B_MaxX
            vx = -abs(vx) end
        if py < B_MinY then py = B_MinY
            vy = abs(vy) end
        if py > B_MaxY then py = B_MaxY
            vy = -abs(vy) end
        if pz < B_MinZ then pz = B_MinZ
            vz = abs(vz) end
        if pz > B_MaxZ then pz = B_MaxZ
            vz = -abs(vz) end
