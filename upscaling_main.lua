...
-- ==========================================
-- 2. ENGINE CONSTANTS & SCALING
-- ==========================================
-- Set this to 0.5 for 2x performance, 0.25 for 4x performance, etc.
...
local function ReinitBuffers(winW, winH)
    -- Calculate internal resolution based on scale
    CANVAS_W = max(1, floor(winW * RENDER_SCALE))
    CANVAS_H = max(1, floor(winH * RENDER_SCALE))
    ...
    -- Set filter to 'nearest' to keep that sharp retro look when scaled
    ...
    -- Adjust FOV to maintain aspect ratio relative to a baseline
    Cam.fov = (CANVAS_W / (800 * RENDER_SCALE)) * 600
end
...
-- ==========================================
-- 4. SCANLINE RASTERIZER (Upscale-Aware)
-- ==========================================
...
function love.draw()
    ...
    -- STRETCH THE IMAGE:
    -- We draw the small image scaled up to fit the window size.
    local winW, winH = love.graphics.getDimensions()
    love.graphics.draw(ScreenImage, 0, 0, 0, winW/CANVAS_W, winH/CANVAS_H)
    
    love.graphics.print("ULTIMA PLATIN | SCALE: "..RENDER_SCALE.." | FPS: "..love.timer.getFPS(), 10, 10)
end
