...
-- 1. Add to GLOBAL STATE, BUFFERS & SOA
...
local lerpT = 0 -- Interpolation timer
...
function love.load()
    ...
    local function CreateAutoSlide(w, h, thickness, color)
        ...
        -- 1. Pathing: Spiral through space
        local angle = sIdx * (math.pi / 2)
        local radius = 1500
        local x = math.cos(angle) * radius
        local y = math.sin(angle) * radius
        local z = sIdx * 3000 -- Increased spacing for the "spiral" feel

        local slide = CreateTriObject(x, y, z, 8, 12, max(w, h))
        slide.color = color or 0xFFD2B48C

        -- 2. Orientation: Make the slide face the path
        Obj_Yaw[slide.id] = angle

        -- 3. Geometry (Standard Cube/Slide)
        ...
        for i, v in ipairs(verts) do
            slide.vx[i-1], slide.vy[i-1], slide.vz[i-1] = v[1], v[2], v[3]
        end
        ...
        for i=0, 11 do
            slide.tris[i] = {v1=indices[i*3+1], v2=indices[i*3+2], v3=indices[i*3+3], color=slide.color}
        end

        -- 4. Waypoint Math: Calculate rotation to face the slide center
        local dist = (h * Cam.fov) / CANVAS_H + 200

        -- Position the camera offset by 'angle'
        local offsetX = math.sin(angle) * dist
        local offsetZ = -math.cos(angle) * dist

        Way_X[sIdx] = x + offsetX
        Way_Y[sIdx] = y
        Way_Z[sIdx] = z + offsetZ

        -- Point the camera back at the slide center (x, y, z)
        -- This automatically handles the 180-degree flip and the spiral turns
        Way_Yaw[sIdx] = math.atan2(-offsetX, -offsetZ)
        Way_Pitch[sIdx] = 0
        -- 5. Initialize camera on first slide
        if sIdx == 0 then
    ...
    -- Invented Setup for first three slides
    CreateAutoSlide(1600, 900, 40, 0xFFFFD700) -- Golden Title Slide
    CreateAutoSlide(1400, 800, 20, 0xFF4682B4) -- Steel Blue Introduction
    CreateAutoSlide(1800, 1000, 60, 0xFF2E8B57) -- Sea Green Technical Details
end

-- 3. Replace love.update with State Branching & Pro Easing
function love.update(dt)
    ...
    if presentationMode and NumSlides > 0 then
        ...
        lerpT = min(1.0, lerpT + dt * 1.5)
        local easeT = 1 - (1 - lerpT) * (1 - lerpT)
        local function lerp(a, b, t) return a + (b - a) * t end

        Cam.pos.x = lerp(Cam.pos.x, Way_X[TargetSlide], easeT)
        Cam.pos.y = lerp(Cam.pos.y, Way_Y[TargetSlide], easeT)
        Cam.pos.z = lerp(Cam.pos.z, Way_Z[TargetSlide], easeT)
        Cam.yaw   = lerp(Cam.yaw, Way_Yaw[TargetSlide], easeT)
        Cam.pitch = lerp(Cam.pitch, Way_Pitch[TargetSlide], easeT)
    else
        -- Free Camera Logic
        local s = 2000 * dt
...
-- 4. Replace love.keypressed with Mode Toggling
function love.keypressed(key)
    ...
    elseif key == "space" and presentationMode then
        TargetSlide = (TargetSlide + 1) % NumSlides
        lerpT = 0
