...
function SlideGuard.deep_merge(base, overlay) if not overlay then return deep_copy(base) end
    local res = deep_copy(base)
    for k, v in pairs(overlay) do if type(v) == "table" and type(res[k]) == "table" then res[k] = SlideGuard.deep_merge(res[k], v)
